...
Codigo_fito<-read.table("./Biologicos/DatosP_Fitoplancton/Matriz_Codigos_Fito2.csv", sep=",", header = TRUE,)
...
amaSumAlta<-as_vector(AmaralesAlta[,5:193]%>% summarise_all(sum))
sanSumAlta<-as_vector(SanquiangaAlta[,5:193]%>% summarise_all(sum))
guaSumAlta<-as_vector(GuascamaAlta[,5:193]%>% summarise_all(sum))

amaSumBaja<-as_vector(AmaralesBaja[,5:193]%>% summarise_all(sum))
sanSumBaja<-as_vector(SanquiangaBaja[,5:193]%>% summarise_all(sum))
guaSumBaja<-as_vector(GuascamaBaja[,5:193]%>% summarise_all(sum))

Transectos<-list(amaSumAlta, sanSumAlta, guaSumAlta, amaSumBaja, sanSumBaja, guaSumBaja)
...
est.Comms <- iNEXT(Transectos, 
                   q=c(0, 1, 2), 
                   datatype = "abundance",
                   endpoint=5000) # q = 0 es la riqueza (diversidades verdaderas)
